...
public class STOTEM {
    ...
    public static String learner = "newRIP";
	...
	for (i = 0; i < num_samples; i++) {
		...
		if((learner.equals("oldRIP")) || (learner.equals("oldRIPovert"))){
		    ...
		    /*
		    // temporary code to compare HG and OT Ripping
		    System.out.println("RIPPING output form: " + output.form);
		    System.out.println("The current optimal form is " + optimal.oform);
		    STOT temp_gr = new STOT(gf);
		    for (int t=0; t<1000; t++){
			GrammarFile.Candidate temp = null;
			double[] temp_single = temp_gr.sample(NegOK, noise);
			int[] temp_rank = temp_gr.find_order(temp_single);
			//System.out.println("Sampled:\t" + gr.gramToString(temp_single));
			if(OT){
			    temp = optimize(tab, input, temp_rank);
			}else{
			    temp = optimizeHG(tab, input, temp_single);
			}
			System.out.println(temp.form);
		    }
		    */
		...
		}else{ // This is either newRIP or randRIP or Baseline
		...
		/* MAGRI's UPDATE
		// This must be wrong because it performs a few points worse than regular GLA and EIP versions
		if(update){
		    // Have to find number of winner-preferring constraints
		    double wnum = 0.0;
		    // Have to find the ranking value of the highest ranked W
		    double Wrank = 0.0;
		    boolean set = false;
		    for(int c=0; c < gr.grammar.length; c++){
			// if c is winner-preferring...
			if(winner.violations[c] < optimal.violations[c]){
			    wnum++;
			    if(orig_single[c] > Wrank || set == false){
				Wrank = single[c];
				set = true;
			    }
			}
		    ...
		    // Have to find number of undominated, loser-preferring constraints
		    double lnum = 0.0;
		    for(int c=0; c < gr.grammar.length; c++){
			if(winner.violations[c] > optimal.violations[c] && orig_single[c] >= Wrank){
			    lnum++;
			}
		    }
		    ...
		    for(int c=0; c < gr.grammar.length; c++){
			if(model.equals("OT")){
			    // if c is loser-preferring and undominated
			    if(winner.violations[c] > optimal.violations[c] && orig_single[c] >= Wrank){
				gr.grammar[c] -= r;
				if(!(NegOK)){
				    if(gr.grammar[c] < 0){
	...
	for (i = 0; i < num_samples; i++) {
	    System.out.println("Starting iteration " + i);
	    int wins = 0;
	    double[] average_ranks = new double[gr.grammar.length];
	    //each iteration consists of s samples
	    for (int s = 0; s < sample_size; s++) {
		...
		//sample a correct ranking for this UR/output
		boolean looking = true;
		int count = 0;
		while ((looking) && (count < 100)){
			...
			//System.out.println("\t\tSampled the correct ranking: " + gr.gramToString(single));
			for(int c=0; c < gr.grammar.length; c++){
			    //record this ranking
			    average_ranks[c] = ((double)(average_ranks[c]*((double)wins-1.0)) + single[c])/((double)wins);
			}
		    }
		}
	    }

	    //done with sampling, update grammar
	    for(int c=0; c < gr.grammar.length; c++){
		gr.grammar[c] = average_ranks[c];
	    }
	    System.out.println("NUmber of wins this iteration was " + wins);
	    System.out.println("The new grammar is:\n" + gr.gramToString(gr.grammar));
	    if (evaluate_grammar(100, i, 1)){
		System.out.println("-reached perfection early ----- exiting now");
		break;
	    }
	    System.out.println("Now evaluating nonnoisy grammar");
	    evaluate_grammar(10, i, 0);
	}
	System.out.println("------------------EVALUATING-------------FINAL----------------GRAMMAR--------------------");
	evaluate_grammar(1000, i, 1);
    }
