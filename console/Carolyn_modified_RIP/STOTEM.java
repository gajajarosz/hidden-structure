...
public class STOTEM {
	...
	public static String graphData = "iterations,accuracy,syllable\n";
	public static String label;

	public static void main(String[] args) {
		if (args.length != 10) {
			System.out.println("usage: java STOTEM grammar_file dist_file sample_size num_samples learning_rate model learner noise NegOK? label");
			System.exit(-1);
		}
	...
	// there are i iterations of sampling and updating
		...
		//sample an output form: output  Look at one data point
		...
		//sample a ur for the output form: input  Look at an underlying form for the output
				String input = "";
				rand = Math.random();
		...
		if(learner.equals("Baseline")){ //Is this the picking a random parse for the baseline?
		...
		if(model.equals("OT")){
		    optimal = optimize(null, input, rank); //use rank for OT--- but why?
		}else if(model.equals("HG")){
		    optimal = optimizeHG(null, input, single); //use single for HG--- why?
		}else{
		    optimal = optimizeME(null, input, gr.grammar);  //MaxEnt uses gr.grammar
		}
		//System.out.println("The current optimal form is " + optimal.oform);
		//System.out.println("The current grammar is:\n" + gr.gramToString(gr.grammar));

		if((learner.equals("oldRIP")) || (learner.equals("oldRIPovert"))){ //oldRIp = RIP; oldRIPovert= RRIP
			...
			//this is an error 
				...
				single = gr.sample(NegOK, noise); //resample grammar?
				//System.out.println("\nSampled:\n" + gr.gramToString(single));
				rank = gr.find_order(single);
				//System.out.println("\t\tSampled the ranking: " + gr.gramToString(rank));
	...
	if(update){
		for(int c=0; c < gr.grammar.length; c++){
			if(model.equals("OT")){
			    if(winner.violations[c] > optimal.violations[c]){ //OT updates weights a fixed amount, HG updates by portion of different X learning rate
				gr.grammar[c] -= r;//is r learning rate? yep.
				if(!(NegOK)){ //don't allow negative weights
				if(gr.grammar[c] < 0){
					gr.grammar[c] = 0;
				}
			}
...
//System.out.println(graphData);
	...
	//System.out.println("ITERATION " + i + ":: Total error is " + error + " and log likelihood is " + log_likelihood);	TO GRAPH
	if (error == 0.0){
		return true;
	}else{
		return false;
...
public static void graph() {
	try {
		Files.write(Paths.get(label + "-" + model + "-" + learner + ".csv"), graphData.getBytes());
